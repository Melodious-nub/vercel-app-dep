  ...
  hasActiveChilds(parent: NavigationDropdown): boolean {
    return parent.children.some((child) => {
      ...
      if (this.isLink(child) && !this.isFunction(child.route)) {
        // Check if the route is active even for child routes
        const routePath = child.route as string;
        return this.router.isActive(routePath, false) || this.router.isActive(`${routePath}/`, false);
      }
